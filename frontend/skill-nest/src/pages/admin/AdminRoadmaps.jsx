...
const AdminRoadmaps = () => {
  ...
  const fetchRoadmaps = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/admin/roadmaps`, {
        method: "GET",
        credentials: "include",
      });
  ...
  const archivedRoadmaps = roadmaps.filter(
    (roadmap) => roadmap.status === "archived",
  ).length;
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Roadmaps</h1>

          <p className="text-gray-500 mt-1">
            Manage learning roadmaps available to learners.
          </p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="font-semibold text-gray-800">All Roadmaps</h2>

          <p className="text-sm text-gray-500 mt-1">
            Create, publish, archive or remove roadmaps.
          </p>
        </div>
                  ...
                  <tr key={roadmap._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-600">
                        {roadmap.businessIdea?.title || "Not linked"}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-2 flex-wrap">
                        ...
                        <button
                          onClick={() =>
                            navigate(`/admin/roadmaps/${roadmap._id}`)
                          }
                        ...
                        <button
                          onClick={() =>
                            navigate(`/admin/roadmaps/${roadmap._id}/edit`)
                          }
                          className="px-3 py-2 text-xs font-medium bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100"
                        >
                          Edit
                        </button>
                        ...
                        {roadmap.status !== "published" &&
                          roadmap.status !== "archived" && (
                            <button
                              disabled={actionLoading === roadmap._id}
                              onClick={() =>
                                updateStatus(roadmap._id, "published")
                              }
                        ...
                        {roadmap.status === "published" && (
                          <button
                            disabled={actionLoading === roadmap._id}
                            onClick={() => updateStatus(roadmap._id, "draft")}
                        ...
                        {roadmap.status !== "archived" && (
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (confirmed) {
                                updateStatus(roadmap._id, "archived");
                              }
                            }}
