...
function Sidebar({showSidebar, setShowSidebar}){
  ...
  const [activeIndex, setActiveIndex] = useState(0);
    return (
        <>
         {showSidebar ? (
        <div className="hidden md:block bg-[#1e3a1e] w-55 fixed top-0 left-0 h-screen z-10">
          ...
          {navItems.map(({ icon: Icon, label }, index) => (
            <div
              ...
              onClick={() => {
                ...
                navigate(`/learner/${label.toLowerCase().replace(/\s+/g, '-')}`);
              }}
        ...
        <div className="hidden md:block bg-[#1e3a1e] fixed top-0 left-0 h-screen z-10">
